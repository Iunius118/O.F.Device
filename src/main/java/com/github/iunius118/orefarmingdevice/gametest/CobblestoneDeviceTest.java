...
public class CobblestoneDeviceTest {
    ...
    public static void registerTestInstance(BootstrapContext<GameTestInstance> context) {
        ...
        TEST_FUNCTIONS.forEach(funcKey -> {
            var testKey = ResourceKey.create(Registries.TEST_INSTANCE, funcKey.identifier());
            context.register(testKey, getCobblestoneDeviceTestInstance(funcKey, testEnvs));
        });
    }
